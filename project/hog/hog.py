...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    i, outcome, sum, flag = 0, 0, 0, 0 #the flag in here is a symbol make sure the loop would finish the whole tour
    while i < num_rolls:               
        i += 1
        outcome = dice()
        if outcome != 1:
            ...
            flag = 1                   #if the value of outcome is 1, the flag should be symbolised, so that the sum could be 1
    if flag == 1:
        sum = 1
    return sum
