...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        #如果self的exit存在，那么他就不为none，满足条件，将他的entrance设为self（该对象）
        if self.exit :
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # problem3:
        # start ant's current place
        # while the place is not hive:
        #   if there are bee(s) and within it's range, return a random bee
        #   update the place to the place's entrance
        # when doing the loop don't change the instance
        # return None
        distance = 0
        current_place = self.place
        while current_place != beehive:
            if distance >= self.min_range and distance <= self.max_range and current_place.bees:
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    max_range, min_range = float('inf'), 5
    armor = 1
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # every single bee in the same place have taken the same damage
        for bee in self.place.bees[:]:
            # bee.armor -= amount
            if self.armor - amount <= 0:
                Insect.reduce_armor(bee, amount + self.damage)
            else: Insect.reduce_armor(bee, amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting != 0:
            self.digesting -= 1
        elif self.place.bees: self.eat_bee(rANTdom_else_none(self.place.bees))
        # END Problem 6
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_watersafe: Insect.reduce_armor(insect, insect.armor)
        # END Problem 8
...
class QueenAnt(ScubaThrower, Counter):  # You should change this line
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.isreal:
            ...
            while current_place:
                if current_place.ant and not current_place.ant.isbuff:
                    current_place.ant.damage *= 2
                    current_place.ant.isbuff = True
                current_place = current_place.exit
        else: self.reduce_armor(self.armor)
        # END Problem EC
